/*!
 * ByteBuffer
 * yoke 2013 https://github.com/yoke233/ByteBuffer
 * fork from yoyo 2012 https://github.com/play175/ByteBuffer
 * new BSD Licensed
 */
...
var ByteBuffer = function(org_buf, offset) {
    ...
    this.initWithList = function(list) {
        _list = list;
        return this;
    };
     ...
     * 可单独设置字符串编码
     **/
    this.vstring = function(val, len, encoding, index) {
        ...
        if (val == undefined || val == null) {
            var vlen = len; //实际长度
            for (var i = _offset + len - 1; i >= _offset; i--) {
                if (_org_buf[i] == 0) {
                    vlen--;
                } else {
                    break;
            ...
            vlen++;
            var buf = _org_buf.slice(_offset, _offset + vlen);
    ...
    this.what4 = function(val, index) {
        if (val == undefined || val == null) {
            var vaa = _org_buf['readInt32' + _endian + 'E'](_offset);
            if (vaa > 1000000000) {
                vaa = _org_buf['readFloat' + _endian + 'E'](_offset);
                vaa = Math.round(vaa * 100) / 100;
            }
            _list.push(vaa);
            _offset += 4;
        } else {
        ...
        for (var i = 0; i < _list.length; i++) {
            switch (_list[i].t) {
                ...
                case Type_String:
                    ...
                    break;
                case Type_VString:
                    var buf = iconv.encode(_list[i].d, _list[i].e);
                    var vlen = buf.length; //字符串实际长度
                    console.log(_list[i].e, buf);
                    buf.copy(_org_buf, offset);
                    //补齐\0
